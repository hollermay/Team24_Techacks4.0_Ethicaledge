...
export default function Sidebar() {
    ...
    if(localStorage.getItem('theme')===undefined || localStorage.getItem('theme')==0){
        tailwnd = 'dark:bg-slate-800 dark:text-white dark:text-slate-400 fixed flex flex-col top-0 left-0 w-64 bg-white h-full shadows'
    }
    else{
        tailwnd = "fixed flex flex-col top-0 left-0 w-64 bg-white h-full shadows"
    }
                            ...
                            return (link.title ? 
                                <li key={i} className="px-5">
                                    <div className="flex flex-row items-center h-8">
                                        <div className="text-sm  tracking-wide ">{link.title}</div>
                                    </div>
                                </li> : 
                                <li key={i}>
                                    <a href={link.navigateto} className={`relative flex flex-row items-center h-11 focus:outline-none hover:bg-gray-50  hover:text-gray-800 border-l-4 border-transparent hover:border-indigo-500 pr-6 ${currentPath == link.navigateto && "bg-gray-0  border-l-4 !border-indigo-500 pr-6"}`}>
